<!--
  ...
  File Name:     NewsSearchBox.vue
  File Function: 宠物新闻搜索框组件
...
<template>
  ...
  <el-menu v-if='showDropdown' class='search-dropdown' v-loading='loading'>
    ...
    <el-menu-item v-for='result in searchResults'
                  :key='result.item.id'
                  @click='router.push(`/news/${result.item.newsId}`)'
...
watch(searchContent, async (newPattern) => {
  if (newPattern.trim() == '') {
    ...
    try {
      const data = await axiosInstance.get(`search/news`)
      list.value = data.data.map(item => ({...item, type: 'type1'}))
      initFuseNews(list.value)
    } catch (error) {
      ElMessage.error(t('ErrorMessage.GetErrorMessage'))
    } finally {
      loading.value = false
      const results = fuseNews.search(newPattern)
