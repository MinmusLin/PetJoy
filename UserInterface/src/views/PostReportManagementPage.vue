<!--
  ...
  File Name:     PostReportManagementPage.vue
  File Function: 帖子举报管理页面
  ...
  <div class='page-container'>
    ...
    <el-table ref='tableRef'
      ...
      <el-table-column prop='title'
                       :label="t('PostCommentReportManagementPage.Post')"
                       width='345'
                       align='center'>
        <template #default='{ row }'>
          <!--suppress TypeScriptUnresolvedReference-->
...
const reportData = ref<PostReport[]>([])
const tableRef = ref()

onMounted(async () => {
  try {
    const response = await axiosInstance.get('post-report/management')
...
const handleStatusClick = async (report) => {
  if (report.status == 0) {
    try {
      await axiosInstance.put(`post-report/${report.postReportId}`, {
