<!--
  ...
  File Name:     NewsCommentReportManagementPage.vue
  File Function: 新闻评论举报管理页面
  ...
  <div class='page-container'>
    ...
    <el-table ref='tableRef'
      ...
      <el-table-column prop='title'
                       :label="t('PostCommentReportManagementPage.ReportedNews')"
                       width='230'
                       align='center'>
        <template #default='{ row }'>
          <!--suppress TypeScriptUnresolvedReference-->
          <span class='hover-blue' @click='navigateToNews(row.reportedNewsId)'>
            {{ row.title }}
          </span>
        </template>
      </el-table-column>
...
type NewsCommentReport = {
  newsCommentReportId: number
  reporterId: number
  reporterName: String
...
const commentReportData = ref<NewsCommentReport[]>([])
const tableRef = ref()

onMounted(async () => {
  try {
    const response = await axiosInstance.get('news-comment-report/management')
...
const handleStatusClick = async (report) => {
  if (report.status == 0) {
    try {
      await axiosInstance.put(`news-comment-report/${report.newsCommentReportId}`, {
        newsCommentReportId: report.newsCommentReportId,
        reporterId: report.reporterId,
        reportedUserId: report.reportedUserId,
        reportedNewsId: report.reportedNewsId,
